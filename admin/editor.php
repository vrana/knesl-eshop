...
as$u=>$V){if($u===""||!$V)break;$wb++;}$xc=($wb?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($j["field"])))."]']; if ($wb > f.selectedIndex) f.selectedIndex = $wb;\"":"");$c.=$xc;echo(count($Hb)>1?html_select("function[$B]",$Hb,$m===null||in_array($m,$Hb)||isset($Hb[$m])?$m:"","functionChange(this);"):nbsp(reset($Hb))).'<td>';$Xb=$b->editInput($_GET["edit"],$j,$c,$W);if($Xb!="")echo$Xb;elseif($j["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$j["length"],$z);foreach($z[1]as$o=>$V){$V=stripcslashes(str_replace("''","'",$V));$za=(is_int($W)?($W>>$o)&1:in_array($V,explode(",",$W),true));echo" <label><input type='checkbox' name='fields[$B][$o]' value='".(1<<$o)."'".($za?' checked':'')."$xc>".h($b->editVal($V,$j)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$j["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$B'$xc>";elseif(($vd=ereg('text|lob',$j["type"]))||ereg("\n",$W)){if($vd&&$s!="sqlite")$c.=" cols='50' rows='12'";else{$L=min(12,substr_count($W,"\n")+1);$c.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$c>".h($W).'</textarea>';}else{$mc=(!ereg('int',$j["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$j["length"],$y)?((ereg("binary",$j["type"])?2:1)*$y[1]+($y[3]?1:0)+($y[2]&&!$j["unsigned"]?1:0)):($Hd[$j["type"]]?$Hd[$j["type"]]+($j["unsigned"]?0:1):0));echo"<input value='".h($W)."'".($mc?" maxlength='$mc'":"").(ereg('char|binary',$j["type"])&&$mc>20?" size='40'":"")."$c>";}}}function
...
<title>',$Ad,'</title>
<link rel="stylesheet" type="text/css" href="',h(preg_replace("~\\?.*~","",ME))."?file=default.css&amp;version=3.5.2-dev",'">
<script type="text/javascript" src="',h(preg_replace("~\\?.*~","",ME))."?file=functions.js&amp;version=3.5.2-dev",'"></script>
';if($b->head()){echo'<link rel="shortcut icon" type="image/x-icon" href="',h(preg_replace("~\\?.*~","",ME))."?file=favicon.ico&amp;version=3.5.2-dev",'" id="favicon">
